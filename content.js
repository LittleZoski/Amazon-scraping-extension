...
class DataExtractor {
  ...
  static extractVariations() {
    ...
    // Amazon's inline twister data scripts may not yet exist at content script run time.
    ...
    // This element is created by Amazon's P.js and may not exist yet; attempt anyway.
        ...
        if (dims && sortedVars.length > 0) {
          ...
          result.validCombinations = sortedVars.map(combo => {
            ...
            combo.forEach((dimValueIdx, pos) => {
              ...
              const val = dims[dimKey] && dims[dimKey][dimValueIdx];
  ...
  static extractSizeChart() {
    // Size chart is pre-rendered inside a .a-popover-preload div in #sizeChartV2Data_feature_div
    // Selector covers both the preload container and any previously opened popover
  ...
  static extractVariationsFromDoc(doc) {
    ...
    // Fallback: swatch thumbnail URLs for colors not in the script data
    const colorDim = dimensions.find(d => d.name === 'color');
    if (colorDim) {
      for (const v of colorDim.values) {
        if (v.swatchImageUrl && !result.colorImages[v.value]) {
