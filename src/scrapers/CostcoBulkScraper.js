/**
 * CostcoBulkScraper - Bulk scraping for Costco.com category/search pages
 * Follows the same pattern as YamiBulkScraper.js and BulkScraper.js
 */

class CostcoBulkScraper {
  constructor() {
  ...
  getVisibleProductCount() {
    const products = CostcoDataExtractor.extractProductLinksFromPage();
    return products.length;
  }

  /**
   * Inject bulk scrape button into category page
   * Uses blue/teal gradient to match CostcoProductScraper
   */
  injectBulkScrapeButton() {
    // Check if button already exists
    if (document.getElementById('costco-bulk-scraper-btn')) {
      return;
    }
    ...
    this.scrapeButton.id = 'costco-bulk-scraper-btn';
  ...
  showBulkScrapeSettings() {
    const allProducts = CostcoDataExtractor.extractProductLinksFromPage();
  ...
  createSettingsModal(allProducts) {
    ...
    // Costco uses data-testid="Text_Price_XXXXXX" for price elements
    const productsWithMetadata = allProducts.map(p => {
      // Try multiple Costco-specific price selectors
      const priceElement = p.element?.querySelector('[data-testid^="Text_Price_"]') ||
                          p.element?.querySelector('[data-testid^="PriceGroup_"]') ||
                          p.element?.querySelector('.MuiTypography-t5');
      const priceText = priceElement?.textContent?.trim();
      const price = CostcoDOMHelpers.parsePrice(priceText);
    ...
    modal.id = 'costco-scraper-settings-modal';
    modal.style.cssText = `
      position: fixed;
      top: 0;
      left: 0;
      ...
      <div style="background: white; border-radius: 16px; padding: 30px; max-width: 500px; width: 90%; box-shadow: 0 20px 60px rgba(0,0,0,0.3);">
        ...
        <div style="margin-bottom: 25px;">
          <div style="display: flex; justify-content: space-between; margin-bottom: 10px;">
            ...
            <span style="color: #0EA5E9; font-weight: 600; font-size: 16px;">${allProducts.length}</span>
          </div>

          <div style="margin: 20px 0;">
            <label style="display: block; color: #666; font-size: 14px; margin-bottom: 10px;">
              Number to Scrape: <span id="count-value" style="color: #0EA5E9; font-weight: 600;">${allProducts.length}</span>
            </label>
  ...
  async bulkScrapeFromPage(allProducts, maxCount, usePriceFilter, minPrice, maxPrice) {
    try {
      ...
      for (let batchStart = 0; batchStart < productLinks.length; batchStart += BATCH_SIZE) {
        if (stopRequested) {
          console.log('Costco scraping stopped by user');
          break;
        }
        ...
        for (let i = 0; i < batchResults.length; i++) {
          ...
          if (result.status === 'fulfilled' && result.value && result.value.title) {
            ...
            if (result.status === 'rejected') {
              console.error('Error scraping Costco product:', result.reason);
            }
          }
        }
        ...
        if (batchEnd < productLinks.length && !stopRequested) {
          await CostcoDOMHelpers.sleep(100);
        }
      }
      ...
      console.error('Costco bulk scraping error:', error);
   ...
   * This allows us to get the fully-rendered DOM
  ...
  async scrapeProductFromLink(linkData) {
    ...
    try {
      ...
      console.error(`Error scraping Costco product ${productID} in tab:`, error);
      return {
        asin: productID,
        url: url,
        scrapedAt: new Date().toISOString(),
  ...
  createProgressIndicator(totalItems) {
    ...
    progressUI.id = 'costco-scraping-progress';
    ...
    progressUI.innerHTML = `
      <h3 style="margin: 0 0 15px 0; color: #333; font-size: 18px;">Scraping Costco Products...</h3>
      <div style="margin-bottom: 15px;">
        ...
        <div style="width: 100%; height: 8px; background: #e0e0e0; border-radius: 4px; overflow: hidden;">
          <div id="progress-bar" style="width: 0%; height: 100%; background: linear-gradient(90deg, #0EA5E9, #06B6D4); transition: width 0.3s;"></div>
        </div>
      </div>
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = CostcoBulkScraper;
}
